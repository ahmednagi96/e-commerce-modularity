...
class PurchaseItems
{
    ...
    public function handle(CartItemCollection $items, PendingPayment $pendingPayment,UserDto $userDTO): OrderDto
    {
        ...
        $orderDto = $this->databaseManager->transaction(function () use ($userDTO, $items, $pendingPayment):OrderDto {
            ...
            $newOrder->fulfill();
            $chrage = $this->chargeAction->handle($pendingPayment->provider, $pendingPayment->payementToken, $newOrder->total_in_cents);
            $this->createPayment->handle(paymentId: $chrage["id"], orderTotal: $newOrder->total_in_cents, userID: $userDTO->userId, orderId: $newOrder->id);
